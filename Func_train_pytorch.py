...
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, size=256, dali_cpu=False, local_rank=0,
                 world_size=1):
...
if __name__ == '__main__':
    ...
    print(
        'datadir: {}, nepochs: {}, lr: {}, bz: {}, num_workers: {}, alpha_ratio: {}, is_prefetch: {},is_focalloss: {}, is_focalloss_alpha: {}'
        .format(data_dir, nepochs, learning_rate, batch_size, num_workers, alpha_ratio, is_prefeacher, is_focalloss,
                is_focalloss_alpha))

    now = datetime.now()
    tm_now = now.strftime("%m%d%H%M")
    ...
    # num_threads=num_workers, crop=224, device_id=0, num_gpus=1)
    # val_loader = get_imagenet_iter_dali(type='val', image_dir=data_dir, batch_size=batch_size,
    # num_threads=num_workers, crop=224, device_id=0, num_gpus=1)o
    data_transforms = {
    ...
    logger.append('class_names: {}'.format(class_names))
    # model parameters
    ...
    dst_model_path = 'resnet_ft_img_{}.pth'.format(data_subfolder + '-' + str(class_nums) + '_' + tm_now)
