...
def fetch(url, subfolder, folder):
    ...
    try:
        ...
        for obj, locs in class_loc.items():  # 单个label
            idx_label = classes.index(obj) #类别id
            for idx, loc in enumerate(locs):
                xmin, ymin, xmax, ymax = loc
                onelabel = '{},{},{},{},{},{},{},{},{}\n'.format(xmin, ymin, xmax, ymin, xmax, ymax, xmin, ymax, obj)
                rst +=onelabel

        if len(loc)>0:
            txt_name=img_name[:len(img_name) - 4] + '.txt'
...
if __name__ == '__main__':
    img_list = open('All.txt', 'rb').readlines()
    imgs = [img.strip() for img in img_list]

    rstdir = 'onlineV1_VOC-to-East'
    tm_start = time.time()
    img_step = 1000
    ...
    for url in imgs:
        ...
        img_name = basic_info['filename']
        img_path = os.path.join('/workspace/autohome/github/tf_faster_rcnn/data/carlogo/JPEGImages', img_name)
