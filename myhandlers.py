...
import errno, logging, socket, os, struct, time, re
...
class TimedRotatingFileHandler(BaseRotatingHandler):
    ...
    def __init__(self, filename, when='h', interval=1, backupCount=0, encoding=None, delay=False, utc=False):
        ...
        if self.when == 'S':
            ...
            self.extMatch = r"^\d{14}$"
        elif self.when == 'M':
            self.interval = 60 # one minute
            self.suffix = "%Y%m%d%H%M"
            self.extMatch = r"^\d{12}$"
        elif self.when == 'H':
            self.interval = 60 * 60 # one hour
            self.suffix = "%Y%m%d%H"
            self.extMatch = r"^\d{10}$"
    ...
    def doRollover(self):
        ...
        currentTime = int(time.mktime(time.strptime(currentTime, "%Y%m%d%H%M%S")))
