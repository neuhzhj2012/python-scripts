...
def parseXml(xml_path):
    ...
    for obj in root.iter('object'):
        clses = obj.find('names')
        cls = clses.find('name').text
...
def parseXmlOne(xml_name,xml_folder, dst_folder):
    try:
        ...
        assert (basic_info['filename'] == xml_name.replace('.xml', ''))
        ...
        writeXml(anno_tree, os.path.join(dst_folder, xml_name.replace('.jpg', '')))
